...
export const testRoleRepository = {
  async findAllByProject(projectId: string): Promise<TestRole[]> {
    const { data, error } = await supabase.from('test_roles').select('*').eq('project_id', projectId).order('name');
    if (error) throw error;
    return (data ?? []).map(mapTestRoleRow);
  },

  async create(input: { projectId: string; name: string }): Promise<TestRole> {
    const { data, error } = await supabase.from('test_roles').insert({ project_id: input.projectId, name: input.name }).select('*').single();
    if (error) throw error;
