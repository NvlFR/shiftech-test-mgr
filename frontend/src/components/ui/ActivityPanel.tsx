...
export function ActivityPanel({ projectId }: { projectId: string }) {
  ...
  useEffect(() => {
    let active = true;
    const reload = () => activityService.listByProject(projectId).then((rows) => { if (active) setEvents(rows); }).catch(() => undefined);
    void reload();
    const channel = supabase.channel(`activity:${projectId}`).on('postgres_changes', { event: '*', schema: 'public', table: 'audit_logs', filter: `project_id=eq.${projectId}` }, reload).subscribe();
