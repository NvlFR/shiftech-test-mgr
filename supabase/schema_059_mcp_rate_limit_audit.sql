-- MCP tool-call governance. Run after schema_058_mcp_analysis.sql.
-- Audit rows intentionally contain no MCP arguments, result payloads, or raw tokens.

alter table ai_audit_events add column if not exists tool_name text;
alter table ai_audit_events alter column action drop not null;
alter table ai_audit_events alter column provider drop not null;
alter table ai_audit_events alter column model drop not null;
alter table ai_audit_events alter column prompt_version drop not null;
alter table ai_audit_events alter column request_hash drop not null;
alter table ai_audit_events drop constraint if exists ai_audit_events_action_check;
alter table ai_audit_events drop constraint if exists ai_audit_events_action_or_tool_check;
alter table ai_audit_events add constraint ai_audit_events_action_or_tool_check
  check (
