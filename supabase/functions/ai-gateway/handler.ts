...
export async function handleAiGateway(request: Request): Promise<Response> {
  ...
  try {
    ...
    const prompt = JSON.stringify({ action, input: JSON.parse(redactPromptInput(input)), scopedContext: JSON.parse(redactPromptInput(context)), promptVersion: PROMPT_VERSION, outputSchema: OUTPUT_SCHEMA_HINT[action], outputInstruction: action === "generate_test_cases" ? "Return a single JSON object that EXACTLY matches outputSchema. Generate separate test-case rows covering happy paths, at least one negative scenario, and at least one edge case. Every row must include requirementRef so it can be traced to its source requirement. Use only the shown keys, no markdown or prose. All results are drafts requiring human review." : "Return a single JSON object that EXACTLY matches the keys, nesting, and enum values shown in outputSchema. Use only those keys — no extra keys, no markdown, no prose. Enum fields (priority, severity, risk) must be one of the listed values. Any id you output must be copied verbatim from scopedContext. All results are drafts requiring human review." });
