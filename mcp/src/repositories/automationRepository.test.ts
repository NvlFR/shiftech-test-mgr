...
test("automation calls scoped RPC and keeps token out of URL", async () => {
  ...
  assert.deepEqual(calls.map((call) => call.url.split("/").at(-1)), ["mcp_map_automation_script", "mcp_enqueue_automation", "mcp_automation_job_status", "mcp_automation_runner_list"]);
