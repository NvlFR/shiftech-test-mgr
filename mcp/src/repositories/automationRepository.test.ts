...
test("automation calls scoped RPC and keeps token out of URL", async () => {
  ...
  const repository = new AutomationRepository(config, transportFor(fetchImpl));
  ...
  await repository.verifyRegression({ issueId: "issue", testRunId: "run" });
  await repository.regressionDiffContext("issue");
  await repository.jobStatus("job"); await repository.runnerList();
  assert.deepEqual(calls.map((call) => call.url.split("/").at(-1)), ["mcp_map_automation_script", "mcp_enqueue_automation", "mcp_rerun_failed_automation", "mcp_verify_regression", "mcp_regression_diff_context", "mcp_automation_job_status", "mcp_automation_runner_list"]);
  ...
  assert.deepEqual(calls[2]!.body.p_diff_paths, []);
  assert.equal(calls[3]!.body.p_test_run_id, "run");
});

test("automation repository redacts upstream failure", async () => {
  const repository = new AutomationRepository(config, transportFor(async () => new Response("secret", { status: 403 })));
