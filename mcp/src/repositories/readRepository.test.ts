...
test("search sends project scope and token in RPC body, then maps snake_case rows", async () => {
  ...
  const rows = await new ReadRepository(config, transportFor(fetchImpl)).searchTestCases({ tag: "smoke", limit: 11 });
...
test("upstream failures are replaced with a credential-safe repository error", async () => {
  const repository = new ReadRepository(config, transportFor(async () => new Response("sensitive upstream detail", { status: 500 })));
...
test("batch 2 result listing sends filters and maps nested result data", async () => {
  ...
  const rows = await new ReadRepository(config, transportFor(fetchImpl)).listTestResults({ status: "fail", testerId: "88888888-8888-4888-8888-888888888888", testRunId: "66666666-6666-4666-8666-666666666666", limit: 11 });
...
test("test run summaries are mapped from on-the-fly RPC output", async () => {
  const repository = new ReadRepository(config, transportFor(async () => new Response(JSON.stringify([{ id: "99999999-9999-4999-8999-999999999999",
...
test("requirement listing maps uncovered requirements", async () => {
  const repository = new ReadRepository(config, transportFor(async () => new Response(JSON.stringify([{ id: "22222222-2222-4222-8222-222222222222",
...
test("artifact signing calls the server-side signer without exposing credentials", async () => {
  ...
  const repository = new ReadRepository(config, transportFor(async (_input, init) => { body = JSON.parse(String(init?.body));
