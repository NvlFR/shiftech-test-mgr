...
test("flaky analysis sends scoped RPC parameters and maps rows", async () => {
  ...
  const repository = new AnalysisRepository(config, async (_input, init) => { body = JSON.parse(String(init?.body)); return new Response(JSON.stringify([{
...
test("analysis upstream details are credential-safe", async () => {
  const repository = new AnalysisRepository(config, async () => new Response("token secret", { status: 500 }));
