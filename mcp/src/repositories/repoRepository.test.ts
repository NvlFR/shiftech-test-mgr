...
test("repository configuration RPC is project scoped and maps credential internally", async () => {
  ...
  const repository = new RepoRepository(config, transportFor(async (_input, init) => {
...
test("repository configuration errors do not expose upstream response", async () => {
  const repository = new RepoRepository(config, transportFor(async () => new Response("credential leak", { status: 500 })));
